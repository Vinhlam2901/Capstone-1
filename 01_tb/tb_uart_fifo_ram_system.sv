...
module tb_uart_fifo_ram_system;
  ...
  initial begin
    $dumpfile("wave.vcd");
    $dumpvars(0, tb_uart_fifo_ram_system);
  end
  ...
  // ===================================================================
  initial begin
    out_file_id = $fopen("output_dog_plus10.hex", "w");
    forever begin
      @(negedge tx);
  ...
  initial begin
    rst_n = 0; rx = 1'b1;
    $readmemh("../01_tb/image_test/dog_gen.hex", gold_mem);
    #100 rst_n = 1; #5000;

    $display("\n[INFO] BAT DAU TRUYEN %0d BYTES...", total_test_bytes);
    
    in_file_id = $fopen("../01_tb/image_test/dog_gen.hex", "r");
